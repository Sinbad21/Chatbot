  ...
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
   <Card className="max-w-2xl w-full max-h-[90vh] overflow-y-auto bg-gradient-to-br from-[#2d1b4e] to-[#150a25] border border-purple-500/20">
    ...
    <div className="sticky top-0 bg-gradient-to-br from-[#2d1b4e] to-[#1a0f2e] border-b border-purple-500/20 p-6 flex items-center justify-between">
     <div>
      <h2 className="text-2xl font-bold text-white">Integrazione Telegram</h2>
      <p className="text-sm text-purple-300/70 mt-1">Step {step} di 3</p>
     </div>
    ...
    <div className="px-6 pt-4">
     <div className="flex gap-2">
      {[1, 2, 3].map((s) => (
       <div
        key={s}
        className={`h-2 flex-1 rounded-full ${s <= step ? 'bg-gradient-to-r from-purple-500 to-fuchsia-500' : 'bg-purple-900/50'}`}
       />
      ))}
     </div>
    </div>
    ...
    <div className="p-6">
     {step === 1 && (
      <div className="space-y-4">
       <h3 className="text-lg font-semibold text-white">1. Crea un Bot Telegram</h3>
       <ol className="list-decimal list-inside space-y-3 text-sm text-purple-300/70">
        <li>Apri Telegram e cerca <code className="bg-purple-900/50 px-2 py-1 rounded text-purple-200">@BotFather</code></li>
        ...
        <li>BotFather ti invierÃ  il <strong className="text-white">Bot Token</strong> - copialo!</li>
       </ol>
       <a
        href="https://core.telegram.org/bots#6-botfather"
        target="_blank"
        rel="noopener noreferrer"
        className="text-fuchsia-400 hover:text-fuchsia-300 text-sm inline-flex items-center transition-colors"
       >
     ...
     {step === 2 && (
      <div className="space-y-4">
       <h3 className="text-lg font-semibold text-white">2. Inserisci le credenziali</h3>
       ...
       <div className="bg-purple-900/30 border border-purple-500/20 rounded-lg p-4">
        <p className="text-sm font-medium text-white mb-2">Webhook URL (configurato automaticamente):</p>
        <div className="flex gap-2">
         <code className="flex-1 px-3 py-2 bg-purple-900/50 border border-purple-500/30 rounded text-xs text-purple-200">
          {webhookUrl}
         </code>
         <Button variant="outline" size="sm" onClick={() => copyToClipboard(webhookUrl)} className="border-purple-500/30 text-purple-300 hover:bg-purple-500/10">
      ...
      <div className="space-y-4">
       <div className="bg-fuchsia-500/10 border border-fuchsia-500/20 rounded-lg p-6 text-center">
        ...
        <p className="text-purple-300/70">
         Il webhook verrÃ  configurato automaticamente al salvataggio.
        </p>
       </div>

       <div className="bg-purple-900/30 border border-purple-500/20 rounded-lg p-4">
        <p className="text-sm font-medium text-white mb-3">Come testare:</p>
        <ol className="list-decimal list-inside space-y-2 text-sm text-purple-300/70">
         ...
         <li>Invia <code className="bg-purple-900/50 px-2 py-1 rounded text-purple-200">/start</code></li>
    ...
    <div className="sticky bottom-0 bg-gradient-to-br from-[#2d1b4e] to-[#1a0f2e] border-t border-purple-500/20 p-6 flex justify-between">
     <Button variant="outline" onClick={step === 1 ? onClose : () => setStep(step - 1)} className="border-purple-500/30 text-purple-300 hover:bg-purple-500/10">
