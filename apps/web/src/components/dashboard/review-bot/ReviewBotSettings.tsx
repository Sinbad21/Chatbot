...
export function ReviewBotSettings({ reviewBot, onClose, onSave }: ReviewBotSettingsProps) {
  const [settings, setSettings] = useState({
    ...
    completedMessage: reviewBot?.completedMessage ?? 'Grazie mille per il tuo tempo! ❤️',
      ...
      <div className="relative w-full max-w-xl bg-gradient-to-br from-[#1a0b2e] to-[#0f0520] border border-purple-500/30 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-purple-500/20">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-6 py-2 bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white rounded-xl font-medium hover:from-purple-500 hover:to-fuchsia-500 transition-all shadow-lg shadow-purple-500/25 inline-flex items-center gap-2 disabled:opacity-50"
          >
            {saving ? (
